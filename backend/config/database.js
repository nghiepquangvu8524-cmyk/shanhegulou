...
function initDatabase() {
  ...
  db.get('SELECT COUNT(*) as count FROM users WHERE id = "demo1"', (err, row) => {
    if (err) return;
    if (row.count === 0) {
      insertDemoUsers();
    }
...
function insertDefaultBuildings() {
  ...
  buildings.forEach(b => {
    stmt.run(b.id, b.name, b.dynasty, b.author, b.poem, b.description, b.lng, b.lat);
  });
  stmt.finalize();
  console.log('✅ 默认建筑数据已插入');
}
...
function insertDemoUsers() {
  ...
  users.forEach(u => {
    stmt.run(u.id, u.name, u.email, u.password, u.avatar, u.join_date, u.favorites);
  });
  stmt.finalize();
  console.log('✅ 默认用户已插入');
}
